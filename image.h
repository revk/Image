...
void ImageWriteGif (Image * i, FILE *f, int back, int trans, char *comment);     // write image as a gif
void ImageWritePNGz (Image * i, FILE *f, int back, int trans, unsigned int dpm, char *comment, int z);   // write image as a png
#define ImageWritePNG(i,fh,back,trans,comment) ImageWritePNGz(i,fh,back,trans,0,comment,9)
void ImageWriteSVG (Image * i, FILE *f, int back, int trans, char *comment, double scale);       // scale is mm per pixel, 0 for no scale just units
